...
# Define a list of user inputs instead of a single TASK
USER_INPUTS = [
    "a slogan for a new line of electric cars.",
    # Add more inputs here if needed
]

@tracer.start_as_current_span(name="ai_agent_group_chat_copywriter_reviewer")
async def main():
    ...
    async with (
        ...
        try:
            # 6. Process each user input using the streaming pattern
            for user_input in USER_INPUTS:
                ...
                # Stream the response with the new pattern
                async for content in chat.invoke_stream():
                    if content.role == AuthorRole.ASSISTANT:
                        role_display = f"{content.role} - {content.name or '*'}"
                        print(f"\n# {role_display}: '", end="", flush=True)
